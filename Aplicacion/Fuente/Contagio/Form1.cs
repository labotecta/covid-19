...
namespace Contagio
{
    public partial class Form1 : Form
    {
        ...
        private void Desactiva(bool que)
        {
            ...
            /*filtro_sanos.Enabled = !que;
            filtro_infectados.Enabled = !que;
            filtro_curados.Enabled = !que;
            filtro_recaida.Enabled = !que;
            filtro_muertos.Enabled = !que;*/
        }
        private void Oculta(bool que)
        {
            ...
            /*filtro_sanos.Visible = !que;
            filtro_infectados.Visible = !que;
            filtro_curados.Visible = !que;
            filtro_recaida.Visible = !que;
            filtro_muertos.Visible = !que;*/
        }
        ...
        private bool LeeTodo()
        {
            try
            {
                ...
                {
                    if (!ImportaPoblacion(F_INDIVIDUOS))
                    {
                        return false;
                    }
                    if (NUMERO_CLUSTERS_DATO != NUMERO_CLUSTERS)
                    {
                        // Poner el número de clusters del caso

                        d_numero_c.Text = string.Format("{0}", NUMERO_CLUSTERS_DATO);
                        Desactiva(true);
                        CreaClusters(true);
                        Desactiva(false);
                    }
        ...
        private bool Parametros()
        {
            try
            {
                ...
                if (NUMERO_CLUSTERS_DATO >= MAX_CLUSTERS)
                {
                    MessageBox.Show(string.Format("El número máximo de clusters es {0}", MAX_CLUSTERS), "Datos de entrada", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
        ...
        private bool CreaPoblacion(bool mensaje)
        {
            ...
            foreach (Individuo indi_i in individuos)
            {
                ...
                if (indi_i.enfermo) enfermos++;
                if (indi_i.hospitalizado) hospitalizados++;
            }
            ...
            for (int nf = 0; nf < N_FOCOS_INICIALES; nf++)
            {
                ...
                while (true)
                {
                    ...
                    {
                        paciente = individuos.ElementAt(i);
                        paciente.estado = 1;
                        infectados++;
                        sanos--;
                        break;
        ...
        private bool CreaClusters(bool mensaje)
        {
            ...
            int k = -1;
            total_individuos_clusters = 0;
            individuos_cluster = new List<Individuo>[NUMERO_CLUSTERS_DATO];
            foreach (Grupo g in grupos)
            {
                k++;
                ...
                for (int i = 0; i < clusters_grupo[k]; i++)
                {
                    ...
                    individuos_cluster[n] = new List<Individuo>();
                    for (int nindi = desde_indi; nindi < individuos.Count; nindi++)
                    {
                        indi = individuos.ElementAt(nindi);
                        if (indi.grupo_ID == g.ID)
                        {
                            if (individuos_cluster[n].Count >= MAX_INDIVIDUOS_CLUSTER)
                            {
                                MessageBox.Show(string.Format("Superado el número máximo de individuos por cluster {0}", MAX_INDIVIDUOS_CLUSTER), "Crear clusters", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                return false;
                            }
                            individuos_cluster[n].Add(indi);
                            indi.cluster = n;
                            total_individuos_clusters++;
                            if (individuos_cluster[n].Count == indi_c)
                            {
                                desde_indi = nindi + 1;
                                break;
                            }
            ...
            if (mensaje)
            {
                ...
                {
                    MessageBox.Show(string.Format("Creados {0:N0} clusters.{1} Con {2:N0} individuos", NUMERO_CLUSTERS, Environment.NewLine, total_individuos_clusters), "Crear clusters", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            linea_estado.Text = string.Empty;
        ...
        private bool TablaVecinos(bool mensaje)
        {
            ...
            for (int i = 0; i < individuos.Count; i++)
            {
                if (i % 100 == 0)
                {
                    linea_estado.Text = string.Format("Creando tabla de vecinos {0} de {1} ", i, individuos.Count);
                    Application.DoEvents();
                }
                if (cancelar)
                {
            ...
            d = ((double)individuos.Count) * individuos.Count;
            double factor = n / d;
            if (mensaje)
            {
                MessageBox.Show(string.Format("Distancia media: {0:f0}{1}Distancia máxima media: {2:f0}{3}Factor vecinas: {4:f3}{5}Máx vecinas: {6}", d_med / d, Environment.NewLine, d_max_med / d, Environment.NewLine, factor, Environment.NewLine, nv_max), "Tabla de vecinos", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            if (factor < 0.12)
            {
                TABLA_VECINOS = true;
            ...
            foreach (Individuo indi in individuos)
            {
                ...
                switch (indi.estado)
                {
                    ...
                    default:
                        ...
                        if (indi.contagio_en_cluster)
                        {
                            muertos_total_clusters++;
                        }
                        break;
                }
                if (indi.enfermo) enfermos++;
                if (indi.hospitalizado) hospitalizados++;
            }
            ActualizaMonitorEstados();
            total_pasos = 0;
            total_recorridos = 0;
        ...
        private void CicloDias()
        {
            ...
            ClustersInicio(sw);
            sw.Close();
            fichero = string.Format(@"{0}\s.log", carpeta);
            ...
            sw.WriteLine(string.Format("Número clusters generados          {0,12:N0} con {1,12:N0} Individuos", NUMERO_CLUSTERS, total_individuos_clusters));
            ...
            for (int i = 0; i < DIAS; i++)
            {
                ...
                if (sanos > 0 || desinmunizados > 0)
                {
                    ...
                    foreach (Individuo indi in individuos)
                    {
                        ...
                        g = grupos.ElementAt(indi.grupo_ID);
                    ...
                    if (DENSIDAD_VECINOS > 1)
                    {
                        ContagiosAmbientales();
                    }
                }
                else
                {
                ...
                foreach (Individuo indi in individuos)
                {
                    ...
                    else if (indi.estado == 1)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (azar.NextDouble() < PROB_ENFERMAR)
                                {
                                    ...
                                    if (azar.NextDouble() < PROB_HOSPITALIZACION)
                                    {
                ...
                if (N_FOCOS_IMPORTADOS > 0 && DIAS_FOCOS_IMPORTADOS > 0 && GRUPO_IMPORTADO != -1)
                {
                    if (i > 0 && i % DIAS_FOCOS_IMPORTADOS == 0)
                    {
                        for (int j = 0; j < N_FOCOS_IMPORTADOS; j++)
                        {
                            ...
                            xt = RADIO * xt;
                            yt = RADIO * yt;
                            Individuo individuo_nuevo = new Individuo(individuos.Count, id_grupo_importar, nombre_grupo_importar, xt, yt, 1, 0, 0, 0, -1, false, false, false);
                ...
                sw.WriteLine(string.Format("{0,3} {1,10} {2,10} {3,10} {4,10} {5,10} {6,10} {7,10:f2} {8,10:f2} {9,10:f1} {10,10:f2} {11,10} {12,10} {13,10} {14,10} {15,10} {16,10} {17,10} {18,10}", i + 1, sanos, infectados, importados, curados, desinmunizados, muertos, recorrido_individuo_activo, contactos_de_riesgo, media_dias_infectado, R0, contagios_cercania, contagios_clusters, infectados_total_clusters, curados_total_clusters, muertos_total_clusters, enfermos, hospitalizados, infectados_ambientales));
                sw.Flush();
        ...
        private string CierraCiclo(StreamWriter sw, TimeSpan dt, string texto, int dias_sim)
        {
            ...
            foreach (Individuo indi in individuos)
            {
                ...
                g_indi = grupos.ElementAt(indi.grupo_ID);
        ...
        private void ClustersInicio(StreamWriter sw)
        {
            ...
            for (int n = 0; n < NUMERO_CLUSTERS; n++)
            {
                ...
                for (int nic = 0; nic < individuos_cluster[n].Count; nic++)
                {
                    estado = individuos_cluster[n].ElementAt(nic).estado;
                    switch (estado)
                    {
                        case 0:
                            num_indi_estado[0]++;
                ...
                int ng = individuos_cluster[n].ElementAt(0).grupo_ID;
                sw.WriteLine(string.Format("{0,7:N0} {1,3:N0} {2,-20} {3,10} {4,10} {5,10} {6,10} {7,10} {8,10}", n + 1, ng, grupos.ElementAt(ng).grupo, individuos_cluster[n].Count, num_indi_estado[0], num_indi_estado[1], num_indi_estado[2], num_indi_estado[3], num_indi_estado[4]));
            }
            long total = 0;
            for (int i = 0; i < 5; i++)
            {
        ...
        private void ClustersFinal(StreamWriter sw)
        {
            ...
            for (int n = 0; n < NUMERO_CLUSTERS; n++)
            {
                ...
                int ng = individuos_cluster[n].ElementAt(0).grupo_ID;
                sw.WriteLine(string.Format("{0,7:N0} {1,3:N0} {2,-20} {3,10} {4,10} {5,10} {6,10} {7,10} {8,10} {9,10} {10,10} {11,10:f2}", n + 1, ng, grupos.ElementAt(ng).grupo, individuos_cluster[n].Count, num_indi_estado[0], num_indi_estado[1], num_indi_estado[2], num_indi_estado[3], num_indi_estado[4], intentos_cluster[n], infectados_cluster[n], intentos_cluster[n] == 0 ? 0 : 100.0 * infectados_cluster[n] / intentos_cluster[n]));
            }
            long total = 0;
            for (int i = 0; i < 5; i++)
            {
        ...
        private int ContagiosDia()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < indi_activos.Count; i++)
                {
                    ...
                    g_indi = grupos.ElementAt(indi_i.grupo_ID);
                    pictg_de_i = g_indi.prob_contagio / 100.0;
                    if (indi_i.hospitalizado)
                    {
                        pictg_a_i = F_PROB_HOSPITALIZADO;
                    }
                    else if (indi_i.enfermo)
                    {
                        pictg_a_i = F_PROB_ENFERMO;
                    }
                    else
                    {
                        pictg_a_i = 1;
                    ...
                    for (int j = i + 1; j < indi_activos.Count; j++)
                    {
                        ...
                        if (d < CONTACTO2)
                        {
                            ...
                            {
                                pictg_a_j = F_PROB_HOSPITALIZADO;
                            }
                            else if (indi_j.enfermo)
                            {
                                pictg_a_j = F_PROB_ENFERMO;
                            }
                            else
                            {
                                pictg_a_j = 1;
                            ...
                            {
                                // Posible contagio de 'i' por 'j'. Probabilidad de que 'j' le contagie por su propensión alcontagio

                                total_contactos_de_riesgo++;
                                if (azar.NextDouble() < pictg_a_j * pictg_de_i * PROB_CONTAGIO)
                                {
                                    // 'i' infectado 
                            ...
                            else if (indi_i.estado == 1 && indi_i.dias_infectado >= CARENCIA_CONTAGIAR && (indi_j.estado == 0 || indi_j.estado == 3))
                            {
                                ...
                                total_contactos_de_riesgo++;
                                g_indi = grupos.ElementAt(indi_j.grupo_ID);
                                pictg_de_j = g_indi.prob_contagio / 100.0;
                                if (azar.NextDouble() < pictg_a_i * pictg_de_j * PROB_CONTAGIO)
                                {
                                    // 'j' infectado 
        ...
        private int ContagiosDiaVecinos()
        {
            ...
            foreach (Individuo indi_aux in individuos)
            {
                if (cancelar || lzo_ex.cancelar)
                {
                    return -1;
            ...
            while (true)
            {
                ...
                foreach (Individuo indi_i in indi_activos)
                {
                    ...
                    {
                        if (!puntero_activos.TryGetValue(vecinas[i, cj], out j)) continue;
                        indi_j = indi_activos.ElementAt(j);
        ...
        private int ContagiosCluster()
        {
            ...
            for (int n = 0; n < NUMERO_CLUSTERS; n++)
            {
                //foreach (Individuo indi_i in individuos_cluster[n])
                for (int i = 0; i < individuos_cluster[n].Count; i++)
                {
                    indi_i = individuos_cluster[n].ElementAt(i);
                    g_indi_i = grupos.ElementAt(indi_i.grupo_ID);
                    pictg_de_i = g_indi_i.prob_contagio / 100.0;
                    if (indi_i.hospitalizado)
                    {
                        pictg_a_i = F_PROB_HOSPITALIZADO;
                    }
                    ...
                    //foreach (Individuo indi_j in individuos_cluster[n])
                    for (int j = 0; j < individuos_cluster[n].Count; j++)
                    {
                        //if (indi_i.ID == indi_j.ID) continue;
                        if (i == j) continue;
                        indi_j = individuos_cluster[n].ElementAt(j);
                        g_indi_j = grupos.ElementAt(indi_j.grupo_ID);
                        pictg_de_j = g_indi_j.prob_contagio / 100.0;
                        if (indi_j.hospitalizado)
                        {
                            pictg_a_j = F_PROB_HOSPITALIZADO;
                        }
                        ...
                        if ((indi_i.estado == 0 || indi_i.estado == 3) && indi_j.estado == 1 && indi_j.dias_infectado >= CARENCIA_CONTAGIAR)
                        {
                            ...
                            intentos_cluster[n]++;
                            if (azar.NextDouble() < pictg_a_j * pictg_de_i * PROB_CONTAGIO_Cd2)
                            {
                                // 'i' infectado 
                        ...
                        else if (indi_i.estado == 1 && indi_i.dias_infectado >= CARENCIA_CONTAGIAR && (indi_j.estado == 0 || indi_j.estado == 3))
                        {
                            ...
                            intentos_cluster[n]++;
                            if (azar.NextDouble() < pictg_a_i * pictg_de_j * PROB_CONTAGIO_Cd2)
                            {
                                // 'j' infectado 
        ...
        private int ContagiosAmbientales()
        {
            double den;
            Individuo indi;
            for (int n = 0; n < individuos.Count; n++)
            {
                den = DensidadVecinos(n);
                if (den > DENSIDAD_VECINOS)
                {
                    indi = individuos.ElementAt(n);
                    if (indi.estado == 0 || indi.estado == 3)
                    {
                        if (azar.NextDouble() < PROB_AMBIENTAL)
                        {
                            // Infectado 

                            infectados_ambientales++;
                            if (indi.estado == 0)
                            {
                                sanos--;
                            }
        ...
        private double DensidadVecinos(int n)
        {
            ...
            {
                indi = individuos.ElementAt(vecinas[n, i]);
                if (indi.estado == 1 && indi.dias_infectado >= CARENCIA_CONTAGIAR)
                {
                    vinf++;
                }
            }
            return (double)vinf / nvecinas[n];
        }
        #endregion
        ...
        private void ActualizaMonitorEstados()
        {
            ...
            v_enfermos.Text = string.Format("{0:N0}", enfermos);
            v_hospital.Text = string.Format("{0:N0}", hospitalizados);
            Application.DoEvents();
        }
        private void B_importa_individuos_Click(object sender, EventArgs e)
        {
            ...
            {
                Desactiva(true);
                string fo = senda_individuos.Text;
                senda_individuos.Text = F_INDIVIDUOS = leefichero.FileName;
                Application.DoEvents();
                if (!ImportaPoblacion(leefichero.FileName))
                {
                    senda_individuos.Text = F_INDIVIDUOS = fo;
                }
                Desactiva(false);
            }
        }
        private bool ImportaPoblacion(string fichero)
        {
            string s;
            string[] sd;
            FileStream fr;
            ...
            for (int i = 0; i < INDIVIDUOS; i++)
            {
                ...
                /*
                // Pasar a muertos los excluidos

                if (estado == 0 && !filtro_sanos.Checked) estado = -1;
                else if (estado == 1 && !filtro_infectados.Checked) estado = -1;
                else if (estado == 2 && !filtro_curados.Checked) estado = -1;
                else if (estado == 3 && !filtro_recaida.Checked) estado = -1;
                else if (estado == -1 && !filtro_muertos.Checked) estado = -1;
                */

                individuos.Add(new Individuo(individuos.Count, grupo_id, sd[1], xi, yi, estado, dias_infectado, dias_curado, indi_enfermados, cluster, contagiado_en_cluster == 1 ? true : false, enfermo == 1 ? true : false, hospitalizado == 1 ? true : false));
                switch (estado)
                {
                    case 0:
                        sanos++;
                ...
                if (enfermo == 1) enfermos++;
                if (hospitalizado == 1) hospitalizados++;
            }
            ...
            individuos_cluster = new List<Individuo>[NUMERO_CLUSTERS];
            int n;
            for (int i = 0; i < NUMERO_CLUSTERS; i++)
            {
                s = sr.ReadLine();
                ...
                individuos_cluster[i] = new List<Individuo>();
                for (int j = 0; j < n; j++)
                {
                    s = sr.ReadLine();
                    individuos_cluster[i].Add(individuos.ElementAt(Convert.ToInt32(s)));
                }
            }
            sr.Close();
            CasoCambiado();
        ...
        private void ExportarPoblacion(string fichero)
        {
            ...
            sw.WriteLine(string.Format("{0}", individuos.Count));
            ...
            for (int i = 0; i < NUMERO_CLUSTERS; i++)
            {
                int ng = individuos_cluster[i].ElementAt(0).grupo_ID;
                sw.WriteLine(string.Format("{0};{1}", ng, individuos_cluster[i].Count));
                for (int j = 0; j < individuos_cluster[i].Count; j++)
                {
                    sw.WriteLine(string.Format("{0}", individuos_cluster[i].ElementAt(j).ID));
                }
            }
            sw.Close();
        }
